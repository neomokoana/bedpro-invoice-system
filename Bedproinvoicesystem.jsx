...
import {
  ...
  CheckCircle2, Circle, Timer, MailCheck, FileEdit, Upload
} from "lucide-react";
...
const InvoicePreviewModal = ({invoice,customers,settings,onClose,onEmailSent,onStatusChange}) => {
  ...
  const handlePrint = () => {
    if (printRef.current) {
      ...
      w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoice ${invoice.number} — Bed Pro</title>
...
.totals{margin-left:auto;width:260px;}.tr{display:flex;justify-content:space-between;padding:8px 0;font-size:13px;color:#777;border-bottom:1px solid #f5f5f5;}
...
@media print{body{padding:30px 40px;}}</style></head><body>${printContent}<script>window.onload=()=>{window.print();}<\/script></body></html>`);
      w.document.close();
    }
  };

  const handleSendEmail=async()=>{setSending(true);await new Promise(r=>setTimeout(r,1600));setSending(false);onStatusChange(invoice.id,"sent");setShowEmail(false);onEmailSent();onClose();};
  return (
    ...
    <div className="app-main settings-page">
      ...
      {tab === "company" && (
        <Card>
          ...
          <Upload value={form.company?.logo||""} onChange={v=>setNested("company","logo",v)}/>
